...
export function ProfitAndLoss() {
  ...
  const incomeKey = t('reports.income')
  const expensesKey = t('reports.expenses')
  const chartData = data?.periods?.map((p) => ({
    month: formatMonth(p.month),
    [incomeKey]: p.income,
    [expensesKey]: p.expenses,
  })) ?? []
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: t('reports.totalIncome'), value: data?.total_income ?? 0, color: 'text-green-600 dark:text-green-400' },
          { label: t('reports.totalExpenses'), value: -(data?.total_expenses ?? 0), color: 'text-red-600 dark:text-red-400' },
          { label: t('reports.netResult'), value: data?.total_net ?? 0, color: (data?.total_net ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' },
        ].map((kpi) => (
          <div key={kpi.label} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
            ...
            <span className={`text-xl font-bold ${kpi.color}`}>{formatMoney(kpi.value, currency)}</span>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-5">
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            ...
            <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} tickFormatter={(v: number) => formatMoney(v, currency)} width={80} />
            <Tooltip formatter={(v: number) => formatMoney(v, currency)} />
            <Legend />
            <Bar dataKey={incomeKey} fill="#16a34a" radius={[2, 2, 0, 0]} />
            <Bar dataKey={expensesKey} fill="#dc2626" radius={[2, 2, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              {[t('reports.month'), t('reports.income'), t('reports.expenses'), t('reports.result')].map((h) => (
