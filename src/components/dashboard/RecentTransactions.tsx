            ...
            return (
              <div key={tx.id} className="flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <Link to={`/transactions/${tx.id}`} className="text-sm text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 truncate block">
                    {tx.description}
                  </Link>
                  <span className="text-xs text-gray-400 dark:text-gray-500">{formatDate(tx.date)}</span>
                </div>
                <MoneyDisplay amount={debit} currency={firstEntry?.currency ?? 'EUR'} className="text-sm shrink-0" />
              </div>
            )
          })}
          {!data?.items?.length && (
            <p className="text-sm text-gray-400 dark:text-gray-500">{t('dashboard.noTransactions')}</p>
          )}
        </div>
      )}
    </div>
